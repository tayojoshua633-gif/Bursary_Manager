...
class GoogleDriveBackupHelper {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // CloudSync-specific methods (separate folder, keeps latest N backups)
  // ─────────────────────────────────────────────────────────────────────────

  static const String _syncFolderName = 'Bursary CloudSync';
  static const int _maxSyncBackups = 5;

  /// Get or create "Bursary CloudSync" folder in Drive
  static Future<String?> _getOrCreateSyncFolder(drive.DriveApi driveApi) async {
    try {
      final query = "name='$_syncFolderName' and mimeType='application/vnd.google-apps.folder' and trashed=false";
      final fileList = await driveApi.files.list(
        q: query,
        spaces: 'drive',
        $fields: 'files(id, name)',
      ...
      final folder = drive.File()
        ...
        ..mimeType = 'application/vnd.google-apps.folder'
        ..description = 'CloudSync folder for Bursary Manager automatic sync';
  ...
  /// Upload a sync backup (temp file) to the CloudSync folder.
  /// Automatically trims old backups to keep only [_maxSyncBackups].
  static Future<Map<String, dynamic>> uploadSyncBackup(String filePath) async {
    try {
      if (!await isSignedIn()) {
      ...
      final folderId = await _getOrCreateSyncFolder(driveApi);
      ...
      final fileName = 'cloudsync_$ts.db';
      final fileBytes = await file.readAsBytes();

      final driveFile = drive.File()
        ..name = fileName
        ..description = 'Bursary Manager CloudSync automatic backup'
        ..mimeType = 'application/x-sqlite3';
      ...
      // Trim old backups — keep only the latest _maxSyncBackups
      if (folderId != null) {
        try {
          String q = "trashed=false and '$folderId' in parents";
          final all = await driveApi.files.list(
            q: q, spaces: 'drive', orderBy: 'modifiedTime desc',
            $fields: 'files(id)',
          );
          final files = all.files ?? [];
          if (files.length > _maxSyncBackups) {
            for (final old in files.skip(_maxSyncBackups)) {
              if (old.id != null) await driveApi.files.delete(old.id!);
            }
          }
        } catch (_) {}
      }
      ...
      client.close();
      return {'success': true, 'message': 'CloudSync backup uploaded successfully', 'fileName': fileName};
    } catch (e) {
      return {'success': false, 'message': 'CloudSync upload failed: $e'};
    }
  }

  /// List sync backups from the CloudSync folder (newest first)
  static Future<List<Map<String, dynamic>>> listSyncBackupsInDrive() async {
    try {
