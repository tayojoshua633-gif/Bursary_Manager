// lib/screens/staff/staff_setup/staff_register_screen.dart
import 'dart:io';
...
class _StaffRegisterScreenState extends State<StaffRegisterScreen> {
  ...
  final _referee1NameController = TextEditingController();
  final _referee1PhoneController = TextEditingController();
  final _referee1RelationshipController = TextEditingController();
  final _referee1AddressController = TextEditingController();

  // Referee 2 controllers
  final _referee2NameController = TextEditingController();
  final _referee2PhoneController = TextEditingController();
  final _referee2RelationshipController = TextEditingController();
  final _referee2AddressController = TextEditingController();
  ...
  final List<JobExperience> _jobExperiences = [];
  ...
  Future<void> _generateStaffId() async {
    final staffId = await _dbHelper.generateStaffId();
    setState(() {
      _generatedStaffId = staffId;
    });
  }
  ...
  Future<void> _pickImage() async {
    ...
    try {
      ...
      if (image != null && mounted) {
        ...
        final safeStaffId = _generatedStaffId.replaceAll('/', '_');
  ...
  Future<void> _saveStaff() async {
    ...
    try {
      final staff = Staff(
        ...
        lga: _lgaController.text.trim().isEmpty
            ? null
      ...
      await _dbHelper.insertStaff(staff.toMap());

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Staff ${staff.fullName} registered successfully!'),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextFormField(
                      ...
                      validator: (v) =>
                          v?.isEmpty == true ? 'Surname is required' : null,
                    ),
                    const SizedBox(height: 12),
                    TextFormField(
                    ...
                    _buildListSection(
                      items: _academicRecords
                          .map((e) =>
                              '${e.certificate} - ${e.schoolName} (${e.yearObtained})')
                          .toList(),
                      onAdd: _addAcademicRecord,
                      onRemove: (i) =>
                          setState(() => _academicRecords.removeAt(i)),
                    ...
                    _buildListSection(
                      items: _jobExperiences
                          .map((e) =>
                              '${e.position} at ${e.organization} (${e.startDate} - ${e.endDate})')
                          .toList(),
                      onAdd: _addJobExperience,
                      onRemove: (i) =>
                          setState(() => _jobExperiences.removeAt(i)),
